...
require_once __DIR__ . '/../lib/config/index.php';
...
class REST
{
    ...
    {
        $user = $this->sendRequest('user-session', array('user' => array('email' => 'rohit.mishra@raweng.com', 'password' => '***REMOVED***')));
        if (isset($user['user'])) {
    ...
    public function createStack()
    {
        ...
        if (isset($stack['stack'])) {
            ...
            $this->headers['api_***'] = (isset($stack['stack']['api_***'])) ? $stack['stack']['api_***'] : '';
            echo "\nStack created.";
        } else {
    ...
    {
        if ($this->headers && isset($this->headers['api_***'])) {
    ...
    public function generateHeaders($headers = array())
    {
        ...
        if (is_array($headers)) {
            ...
            if (is_array($headers)) {
                foreach ($headers as $*** => $val) array_push($transformedHeader, $*** . ':' . $val);
            }
        }
