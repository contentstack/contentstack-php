    ...
    public function testFindAssetsContainedIn() {
       ...
       /* $_assets = self::$Stack->Assets()->Query()->toJSON()->find();
        ...
        foreach ($assets[0] as $key => $val) {
            $this->assertTrue((array_search($assets[0][$key]['title'], $_set) !== false));
        }
    }
     ...
     public function testFindAssetsNotContainedIn() {
        ...
        foreach ($_assets[0] as $key => $val) {
            $this->assertTrue((array_search($_assets[0][$key]['title'], $_set) === false));
        }
    }
...
/*
    public function testFindAssetsExists() {
        ...
        $this->assertTrue((count($_assets[0]) === $assets_count));
        foreach ($assets[0] as $key => $val) {
            $this->assertTrue(isset($_assets[0][$key]['boolean']));
        }
    }
*/
...
//            $flag = $flag && (count(array_keys($_entries[0][$i])) === 3 && isset($_entries[0][$i]['updated_at']) && isset($_entries[0][$i]['title']) && isset($_entries[0][$i]['uid']));
    ...
    public function testFindOnlyReference() {
        ...
        $flag = true;
        for($i = 0; $i < count($_entries[0]); $i++) {
            $flag = $flag && (count(array_keys($_entries[0][$i]['reference'][0])) === 3);
